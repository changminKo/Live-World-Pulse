...
### 2.3 지구 표면 대비 계약 (확정 — 2026-08-21)

**계약: 지구 표면(해양·육지) vs 우주 배경 luma 차이 ΔL ≥ 22.**
L = Rec.709 luma `0.2126R + 0.7152G + 0.0722B` (감마 공간, 0-255).

- **WCAG 대비 공식을 쓰지 않는다** — 목적 불일치: WCAG는 텍스트 판독용이고 +0.05 flare 항이 근흑 영역을 지배해, 우주 배경(#0c0c0c) 대비 2:1에 표면 #444444(luma 68)가 필요해진다. 이 계약의 목적은 '지구 원반이 우주와 다른 물체로 보이는가'이지 텍스트 가독성이 아니다.
- **측정법**: 실행 스크린샷 픽셀 — 표면(해양·야간면 대용 반구) 4점 이상 / 우주 배경 4점 이상, 점당 3×3 중앙값(별·마커 오염 제거) 후 클래스별 중앙값. 자동화 = `web/scripts/verify-contrast.mjs` (종료 코드 단정).
- **위반 기록 (수정 전, 2026-08-21 실측)**: OpenFreeMap dark 기본값은 background(육지) rgb(12,12,12) = 우주 배경과 **동일색(ΔL 0)**, water rgb(27,27,29) = ΔL 15 — 마커가 허공에 떠 보이는 원인.
- **수정 (런타임 paint 오버라이드 — 스타일 포크·자체 호스팅 없음, `web/src/world/map/surface-contrast.ts`)**: 육지 `#242830`(L 39.7, ΔL 27.7) / 해양·waterway `#1f232b`(L 34.7, ΔL 22.7) / 빙붕·빙하 `#2b2f38`(L 46.8). 육지 > 해양 상대 단차 유지 (기본 스타일은 반대였음).
- **그라티큘**: 30° 경위선, maplibre 네이티브 line (globe 위 선은 네이티브만 — CLAUDE.md 하드 룰). `#9e9e9e`(text-lo) α0.09, 적도·본초자오선만 α0.16. 표면보다 살짝 밝고 마커보다 훨씬 어둡게 — 데이터가 주인공, 마커 가독성을 해치면 실패.
- **부수 조건**: 마커 rank 색(§2.2)은 새 표면 기준으로도 식별 가능해야 함 (스크린샷 육안 판정 항목).
