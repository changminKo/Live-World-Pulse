...
/** 셀 최빈 placeName — 동률은 사전순 최소 (결정론: 내용 해시 안정) */
function modePlace(counts: Map<string, number>): string | null {
 ...
 *  NumMentions는 셀 대표 sampleUrl(가장 많이 언급된 이벤트의 기사) 선택에만 쓴다.
 *
...
export function buildNewsRecords(
  ...
  while (lineStart < csvText.length) {
    ...
    const cols = pickColumns(line, WANTED_COLS);
    if (!cols) {
      dropped += 1;
      continue;
    }
    ...
    if (!cell) {
      cell = { lonIdx, latIdx, articleSum: 0, placeCounts: new Map(), sampleUrl: null, sampleMentions: -1 };
      cells.set(key, cell);
    }
    const articles = articlesStr === '' ? 0 : Number(articlesStr);
    cell.articleSum += Number.isFinite(articles) && articles > 0 ? articles : 0;
    const place = (fullName ?? '').trim();
    if (place !== '') cell.placeCounts.set(place, (cell.placeCounts.get(place) ?? 0) + 1);
    const mentions = mentionsStr === '' ? 0 : Number(mentionsStr);
    const url = (sourceUrl ?? '').trim();
    if (url !== '' && Number.isFinite(mentions) && mentions > cell.sampleMentions) {
      cell.sampleUrl = url;
  ...
  for (const cell of cells.values()) {
    ...
    const payload: NewsPayload = {
      type: 'news',
      placeName: modePlace(cell.placeCounts),
