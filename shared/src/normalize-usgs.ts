...
/** USGS all_hour 정규화 — collector/src/sources/usgs.ts에서 승격 (수정 태스크 Med5).
 *  LIVE(web 직접 fetch, 유일 CORS 예외)와 히스토리(collector 수집)가 동일 함수를 쓴다
 *  (PLAN §8.4 동일 정규화 계약 — drift 금지). 출력 바이트는 collector 기존 구현과 동일. */
