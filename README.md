...
- **항공기** — 6개 지역(서울·도쿄·런던·프랑크푸르트·뉴욕·LA) ADS-B, 방위각 회전 메시, 지역당 10분 주기. ⚠ Cloudflare Workers 공유 IP가 adsb.lol 스로틀에 걸려 실효 수집률 30~50% — 빈 지역은 갭으로 정직 표시
- **기상 경보** — GDACS Green/Orange/Red 활성 경보 400여 건(60분 사이클). 태풍(TC)은 중심 트랙 선 + 예보 불확실성 콘(빗금 폴리곤), 그 밖의 경보는 점
- **뉴스 밀도** — GDELT 15분 export 파일을 0.5° 셀로 집계, 기사 수 로그 스케일 크기
...
실측: 4레이어 동시 렌더 @ 120fps (항공기 최대 2,100여 대 관측) (M5, 1440×900 기준. 렌더 한계는 별도 스파이크에서 30,000점 @ 120fps 검증 — [판정 기록](docs/spike/RESULT.md)).

## 아키텍처 — 월 $0 운영

```
USGS / adsb.lol /
GDACS / GDELT   ──→ Cloudflare Workers Cron (1분, 분→작업 1개)
                         │  정규화 + 멱등 versioned 키
                         ▼
